                ...
                <div class="col-md-12">
                    <form action="{{ route('fleet.reportScreen_post') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                <label for="listdata">Select File:<span class="text-danger required-asterisk">*</span></label>
                                <select name="listdata" id="listdata" class="form-control" {{ empty(old('type', $selectedType ?? '')) ? 'disabled' : '' }}>
                                    <option value="">Select File</option>
                                    @if ($filesList->isNotEmpty())
                                    @foreach ($filesList as $file)
                                    <option value="{{ $file->id }}" {{ old('listdata', $listdata ?? '') == $file->id ? 'selected' : '' }}>
                                        {{ $file->file_name }}
                                    </option>
                                    @endforeach
                                    @endif
